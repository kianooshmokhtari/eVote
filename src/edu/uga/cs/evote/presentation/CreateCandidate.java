...
public class CreateCandidate extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			throw new ServletException("CreateCandidate.doPost: Can't load template in: " + templateDir + ": " + e.toString());
		}
        ...
        //
        candidateName = req.getParameter( "candidateName" );
        if( candidateName == null || candidateName.length() < 1) {
            EvoteError.error( cfg, toClient, "Invalid name input" );
            return;
        }
        candidateParty =  req.getParameter( "candidateParty" );
       
        candidateOffice = req.getParameter( "candidateOffice" );
        if( candidateOffice == null || candidateOffice.length() < 1) {
            EvoteError.error( cfg, toClient, "Invalid office input" );
            return;
        }
        
        try{
        	id = logicLayer.createCandidate(candidateName, candidateParty, candidateOffice);
        }
        catch ( Exception e ) {
