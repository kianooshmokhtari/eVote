package edu.uga.cs.evote.presentation;import java.io.*;
...
public class CreateIssue extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			throw new ServletException("CreateIssue.doGet: Can't load template in: " + templateDir + ": " + e.toString());
		}
        ...
        //
        issueBallotID = req.getParameter( "issueBallotID" );
        if( issueBallotID == null || issueBallotID.length() < 1) {
        	EvoteError.error( cfg, toClient, "Invalid ballot ID input" );
            return;
        }
        
        issueQuestion = req.getParameter( "issueQuestion" );
        if( issueQuestion == null || issueQuestion.length() < 1) {
        	EvoteError.error( cfg, toClient, "Invalid question input" );
            return;
        }
        
        try {
            id = logicLayer.createIssue(issueQuestion, issueBallotID);
        } 
        catch ( Exception e ) {
        ...
        //
        root.put( "issueBallotID", new Long( issueBallotID ) );
